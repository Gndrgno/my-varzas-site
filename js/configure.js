...
function convertTextToHTML(text) {
  // Экранируем HTML-символы, НО не трогаем <u> вручную — сначала обрабатываем теги
  const formatted = text
    .replace(/\*\*(.*?)\*\*/g, "<b>$1</b>")     // **жирный**
    .replace(/_(.*?)_/g, "<i>$1</i>")           // _курсив_
    .replace(/<u>(.*?)<\/u>/g, "<u>$1</u>");    // <u>подчеркивание</u>

  // Теперь экранируем всё остальное (включая < >), чтобы не допустить XSS
  const safeFormatted = formatted
    .replace(/&/g, "&amp;")
    .replace(/</g, "&lt;")
    .replace(/>/g, "&gt;");

  // Вставляем обратно разрешённые теги (b, i, u)
  const unescaped = safeFormatted
    .replace(/&lt;(\/?b)&gt;/g, "<$1>")
    .replace(/&lt;(\/?i)&gt;/g, "<$1>")
    .replace(/&lt;(\/?u)&gt;/g, "<$1>");
            ...
            const commentHTML = `
              <div class="post-comment">
                <div class="comments-data">
                  <p class="username">>> ${finalUsername}</p>
                  <p style="font-weight: 400;">/</p>
                  <p class="post-time comment-time">${commentTime}</p>
                  <p style="font-weight: 400;">/</p>
                  <a href="#" class="comment-id">№${commentId}</a>
                  <p style="font-weight: 400;">/</p>
                  <a class="reply-comment" style="cursor: pointer; color: rgb(62,162,0);">[Ответить]</a>
          ...
          const commentHTML = `
            <div class="post-comment">
              <div class="comments-data">
                ...
                <p style="font-weight: 400;">/</p>
                <p class="post-time comment-time">${comment.time}</p>
