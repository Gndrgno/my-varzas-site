  ...
  const observer = new MutationObserver(mutations => {
    for (const mutation of mutations) {
      for (const node of mutation.addedNodes) {
        ...
        const isComment = node.classList.contains("post-comment") || node.querySelector(".post-comment");
        ...
        if (isComment) {
          const processComment = commentRoot => {
            const username = commentRoot.querySelector(".username");
            const postTime = commentRoot.querySelector(".post-time");
            const commentId = commentRoot.querySelector(".comment-id");
            const replyComment = commentRoot.querySelector(".reply-comment");
            const commentText = commentRoot.querySelector(".comment-text");

            if (username) username.style.color = "rgb(62, 162, 0)";
            if (postTime) postTime.style.color = "rgb(62, 162, 0)";
            if (commentId) {
              commentId.style.color = "rgb(62, 162, 0)";
            }
            if (replyComment) replyComment.style.color = "#59ff00ff";
            if (commentText) commentText.style.color = "white";
          };

          if (node.classList.contains("post-comment")) {
            processComment(node);
          } else {
            const comments = node.querySelectorAll(".post-comment");
